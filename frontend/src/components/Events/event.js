...
const Events = () => {
  ...
  return (
    <div>
      <div class="exeventsoverlay">
        <div class="exeventsoverlay-text">
          <h1>4th International Congress on Women's Health and Midwifery Excellence</h1>
          <div class="exevent-details">
            <div><i class="fa-solid fa-calendar-alt exicon"></i> October 09-10, 2025</div>
            <div><i class="fa-solid fa-map-marker-alt exicon"></i> Toronto, Canada</div>
          </div>
          <div class="exbutton-group">
            <button class="exevent-button">Brochure</button>
            <button class="exevent-button">Abstract</button>
          </div>
        </div>
        <div class="excountdown-container">
          <div class="excountdown-timer">
            <div class="extime-box"><span>{timeLeft.days}</span><span>Days</span></div>
            <div class="extime-box"><span>{timeLeft.hours}</span><span>Hours</span></div>
            <div class="extime-box"><span>{timeLeft.minutes}</span><span>Minutes</span></div>
            <div class="extime-box"><span>{timeLeft.seconds}</span><span>Seconds</span></div>
          </div>
        </div>
      </div>
      
      <div className="exconference-details">
        <div className="eximage-container"> <img src={require('../images/conf.jpg')} alt="Conference" /> </div>
        <div className="extext-container">
          <h2>About the Conference</h2>
          <p> Join us for the 4th International Conference on Women's Health and Midwifery, to be held on October 09-10, 2025, in Toronto, Canada. This conference offers an exceptional opportunity for healthcare professionals, researchers, midwives, and advocates from diverse backgrounds and cultures to engage, collaborate, and explore crucial topics in women’s health and midwifery. <strong> "Empowering Women's Health: Bridging Gaps in Midwifery and Holistic Care" </strong>. </p>
          <p> The forum serves as a dynamic platform for addressing key issues such as maternal and child health, reproductive rights, healthcare access, and the critical role of midwifery in empowering women globally. It also aims to promote advancements in medical education, policy development, and innovative healthcare solutions tailored to women’s unique needs.  </p>
          <p> By participating in this conference, you’ll have the opportunity to showcase your expertise, share success stories, and exchange groundbreaking ideas with a global community of like-minded professionals committed to improving women’s health and well-being.  </p>
          <p>  We invite you to join this inclusive platform to contribute to meaningful discussions, foster collaboration, and make a tangible impact on the future of women's health and midwifery. Be part of the movement toward a healthier, more equitable future for women worldwide! </p>
        </div>
      </div>
      ...
      <div className="exscientific-agenda">
        ...
        <div className="exday-buttons">
          <button onClick={() => setSelectedDay('day1')} className={selectedDay === 'day1' ? 'active' : ''}>  Day 1 (Physical ) </button>
          <button onClick={() => setSelectedDay('day2')} className={selectedDay === 'day2' ? 'active' : ''}>  Day 2 (Physical ) </button>
          <button onClick={() => setSelectedDay('day3')} className={selectedDay === 'day3' ? 'active' : ''}>  Day 1 (Virtual ) </button>
          <button onClick={() => setSelectedDay('day4')} className={selectedDay === 'day4' ? 'active' : ''}> Day 2 (Virtual ) </button>
        </div>
